...
import { fileUpload, del } from "@/api/upload/upload";
...
export default {
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.$modal
        .confirm('确认要"' + text + '"公告吗？')
        .then(function () {
          return changeStatus({ noticeId: row.noticeId });
        })
        .then(() => {
    ...
    handleUpdate(row) {
      ...
      getNotice(noticeId).then((response) => {
        ...
        this.open = true;
        this.title = "修改公告";
        if (this.form.ossIdList && this.form.ossIdList.length) {
          this.form.ossList.forEach((item) => {
            this.fileList.push({
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          if (this.form.noticeId != undefined) {
            if (
              this.form.ossList &&
              this.form.ossList.length != this.fileList.length
            ) {
              this.form.ossList = null;
              this.form.ossIdList = null;
            }
            if (
              this.fileList.length &&
              this.form.ossList &&
              this.form.ossList.length
            ) {
              let urls = this.form.ossList.map((val) => val.url);
