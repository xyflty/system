<template>
  <div class="app-container">
    ...
    <el-card class="box-card" style="margin-top: 20px">
      <div>
        <rank_visit class="chart-box" :initData="echartData" />
      </div>
    </el-card>
    ...
    <el-card class="box-card" style="margin-top: 20px">
        ...
        <div class="msg-box2">
          <span
            ...
            <span class="ableClick">{{
              totalInfo.followUpCountNum
            }}</span></span
          ><span
            >面谈次数：
            <span class="ableClick">{{
              totalInfo.faceTalkCountNum
            }}</span></span
          ><span
            >电话次数：
            <span class="ableClick">{{
              totalInfo.phoneTalkCountNum
            }}</span></span
          >
        </div>
      ...
      <div>
        ...
        >
          ...
          <el-table-column align="center" prop="faceTalkCount" label="面谈次数">
          </el-table-column>
          <el-table-column
            align="center"
            prop="phoneTalkCount"
...
<script>
import rank_visit from "../components/rank_visit.vue";
...
import { getFollowUpRank, getFollowUpRankList } from "@/api/report/index";
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      getFollowUpRank(query).then((res) => {
        this.echartData = [
          res.data.xname,
          res.data.followUpCount,
          res.data.faceTalkCount,
          res.data.phoneTalkCount,
        ];
      });
    },
    getList() {
      ...
      query.userIds = query.userIds.join(",");
      this.loading = true;
      getFollowUpRankList(query).then((res) => {
        this.total = res.data.followUpRankPage.total;
        this.totalInfo = res.data;
        this.list = res.data.followUpRankPage.records;
