  ...
  <div class="container">
    <div class="header">
      ...
      >
        ...
        <el-form-item label="安证有效时间" prop="safetyPermitsTime">
          <el-date-picker
            v-model="queryParams.safetyPermitsTime"
            type="monthrange"
            range-separator="至"
        ...
        <el-form-item label="资质有效期" prop="expireDate">
          <el-date-picker
    ...
    <div class="content mt_20">
      <el-row :gutter="10" class="mb8">
        ...
        <el-col :span="1.5" v-if="queryParams.isReceive == 0">
          <el-button
            ...
            @click="handleImport"
            v-hasPermi="['core:clueSea:import']"
            >导入</el-button
          >
        </el-col> -->
        <!-- <el-col :span="1.5">
      ...
      <el-col :span="1.5">
        ...
        <el-col :span="1.5" v-if="queryParams.isReceive == 0">
          <el-button
            ...
            @click="handleExport"
            v-hasPermi="['core:clueSea:export']"
            >导出</el-button
          >
        </el-col>-->
        ...
        <div v-show="queryParams.isReceive == 0">
          ...
          <el-col :span="1.5">
            <el-button
              ...
              v-hasPermi="['core:clueSea:allotResource']"
              >分配资源</el-button
            >
          </el-col>
          <el-col :span="1.5">
          ...
          <template slot-scope="scope">
            <div v-if="queryParams.isReceive == 1">
              <el-button
                ...
                v-hasPermi="['core:clueSea:convert']"
                >转入线索</el-button
              >
              <el-button
                size="mini"
              ...
              <el-button
                ...
                v-hasPermi="['core:clueSea:putBackSea']"
                >放回公海</el-button
              >
              <el-button
                size="mini"
                ...
                @click="
                  $router.push({
                    ...
                    query: {
                      id: scope.row.id,
                      type: 'clueSea',
                      archiveId: scope.row.archiveId,
                    },
                  })
                "
                v-hasPermi="['core:clueSea:query']"
                >详细</el-button
              >
            </div>
          </template>
      ...
      <div class="info-box" :style="dialogPosition">
        <el-timeline v-if="followUpVoList.length">
          <el-timeline-item
            placement="top"
            color="#0079fe"
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: 10, label: `跟进记录`, visible: true, width: "140" },
      ],
      ...
      rules: {
        ...
        dataSource: [
          { required: true, message: "资源来源 不能为空", trigger: "blur" },
        ],
        province: [
          { required: true, message: "所在地区不能为空", trigger: "blur" },
        ],
        firstReceivePerson: [
          { required: true, message: "首次成交人不能为空", trigger: "blur" },
        ],
        lastReceivePerson: [
          { required: true, message: "最后成交人不能为空", trigger: "blur" },
        ],
        lastFollowUpTime: [
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/zw/clueSea/importData",
      },
  ...
  methods: {
      ...
      if (type == 1 || type == 3) {
        ...
        this.$modal
          ...
          .then(() => {
            this.loading = true;
            return type == 1
              ? convertCustomer({ archiveId: row.archiveId, id: row.id })
              : putBack({ id: row.id });
          })
          .then(() => {
            this.loading = false;
    ...
    getList() {
      ...
      listClueSea(sendParams).then((response) => {
        this.clueSeaList = response.rows;
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除线索公海编号为"' + ids + '"的数据项？')
        .then(() => {
          this.loading = true;
          return delClueSea(ids);
        })
        .then(() => {
          this.loading = false;
          this.getList();
    ...
    handleExport() {
      this.download(
        ...
        `clueSea_${new Date().getTime()}.xlsx`
      );
    },

    /** 导入按钮操作 */
    handleImport() {
      this.upload.title = "线索公海导入";
    ...
    importTemplate() {
      importTemplate().then((response) => {
        ...
        link.download = new Date().getTime() + "线索公海导入模板";
    ...
    feedbackSubmit(model, hide) {
      ...
      let sendData = {
        ...
        archiveId: this.rowItem.archiveId,
        ...model,
      };
