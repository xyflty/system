      ...
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5" v-if="queryParams.isReceive == 0">
          <el-button
            ...
            size="small"
            @click="handleAdd"
            v-hasPermi="['core:commonResourceSea:add']"
            >新增</el-button
          >
        </el-col>
        <el-col :span="1.5" v-if="queryParams.isReceive == 0">
          <el-button
            ...
            size="small"
            @click="handleImport"
            v-hasPermi="['core:resourceSea:import']"
            >导入</el-button
          >
        </el-col>
        <!-- <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['core:commonResourceSea:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['core:commonResourceSea:remove']"
          >删除</el-button
        >
      </el-col> 
        <el-col :span="1.5" v-if="queryParams.isReceive == 0">
          <el-button
            ...
            @click="handleExport"
            v-hasPermi="['core:commonResourceSea:export']"
            >导出</el-button
          >
        </el-col>-->
        ...
        <div v-show="queryParams.isReceive == 0">
          <el-col :span="1.5">
            <el-button
              ...
              @click="handleReceive"
              size="small"
              v-hasPermi="['core:commonResourceSea:take']"
              >领取资源</el-button
            >
          </el-col>
          <!-- <el-col :span="1.5">
            <el-button
              plain
              @click="allotResource"
              v-hasPermi="['core:commonResourceSea:allotResource']"
              >分配资源</el-button
            >
          </el-col> -->
          <!-- <el-col :span="1.5">
          ...
          <template slot-scope="scope">
            <div v-if="queryParams.isReceive == 1">
              <el-button
                ...
                v-hasPermi="['core:commonResourceSea:convert']"
                >转入线索</el-button
              >
              <el-button
                size="mini"
              ...
              <el-button
                ...
                v-hasPermi="['core:commonResourceSea:putBackSea']"
                >放回公海</el-button
              >
            </div>
            <!-- <el-button
            ...
            <div v-else>
              ...
              <el-button
                ...
                @click="
                  $router.push({
                    name: 'SeaDetail',
                    query: { id: scope.row.id, type: 'commonResourceSea' },
                  })
                "
                v-hasPermi="['core:commonResourceSea:query']"
                >详细</el-button
              >
            </div>
          </template>
...
<script>
import {
  listCommonResourceSea,
  getCommonResourceSea,
  delCommonResourceSea,
  addCommonResourceSea,
  updateCommonResourceSea,
  importTemplate,
  receive,
  getPopup,
  convertClue,
  ...
  data() {
    return {
      ...
      columns: [
        ...
        // { key: 4, label: `所在地区`, visible: true, width: "250" },
        { key: 5, label: `首次领取人`, visible: true, width: "180" },
        { key: 6, label: `最后领取人`, visible: true, width: "140" },
        { key: 7, label: `成立时间`, visible: true, width: "180" },
      ],
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/zw/commonResourceSea/importData",
      },
  ...
  methods: {
    handleAction(row, type) {
      if (type == 1 || type == 3) {
        ...
        this.$router.push({
          name: "SeaDetail",
          query: { id: row.id, type: "commonResourceSea", val: 1 },
        });
      }
    },
    ...
    cellMLeave(row, column, cell, event) {
    ...
    getList() {
      this.loading = true;
      listCommonResourceSea(this.queryParams).then((response) => {
        this.commonResourceSeaList = response.rows;
        this.total = response.total;
        this.badgeNum = response.param.receiveCount;
        this.loading = false;
      });
    },
    // 取消按钮
    ...
    reset() {
      this.form = {
        ...
        dataSource: undefined,
        area: undefined,
        firstReceivePerson: undefined,
        lastReceivePerson: undefined,
        createBy: undefined,
    ...
    handleUpdate(row) {
      ...
      // getCommonResourceSea(id).then((response) => {
      this.loading = false;
      this.form = row;
      this.open = true;
      this.title = "修改公共资源公海";
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateCommonResourceSea(this.form)
              .then((response) => {
            ...
            addCommonResourceSea(this.form)
              .then((response) => {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除公共资源公海编号为"' + ids + '"的数据项？')
        .then(() => {
          this.loading = true;
          return delCommonResourceSea(ids);
        })
        .then(() => {
          this.loading = false;
          this.getList();
    ...
    handleExport() {
      this.download(
        "core/commonResourceSea/export",
        {
          ...this.queryParams,
        },
        `commonResourceSea_${new Date().getTime()}.xlsx`
      );
    },

    /** 导入按钮操作 */
    handleImport() {
      this.upload.title = "公共资源库公海导入";
    ...
    importTemplate() {
      importTemplate().then((response) => {
        ...
        link.download = new Date().getTime() + "公共资源库公海导入模板";
